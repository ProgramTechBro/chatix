...
class _ReelsPageViewState extends State<ReelsPageView> {
  ...
  VideoPlayerController _createController(int index) {
    final controller = VideoPlayerController.networkUrl(Uri.parse(widget.videos[index].videoUrl));
  ...
  void _preloadAround(int index) {
    final keep = {index - 1, index, index + 1}..removeWhere((i) => i < 0 || i >= widget.videos.length);
    for (final i in keep) {
      _controllers.putIfAbsent(i, () => _createController(i));
    }
    final stale = _controllers.keys.where((key) => !keep.contains(key)).toList();
