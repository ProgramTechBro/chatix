      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            contactsAsync.when(
              data: (contacts) {
                final filtered = contacts
                    .where((contact) => contact.name.toLowerCase().contains(_query))
                    .toList();
            ...
            Text(
              'Write a message...',
              style: textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w500, color: AppColors.textSecondary),
            ),
            const SizedBox(height: 16),
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 20),
              child: AppButton(
                ...
                onPressed: () {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Sent to ${_selectedIds.length} people')),
                  );
                },
              ),
            ),
...
class _ContactRow extends StatelessWidget {
  const _ContactRow({required this.contact, required this.isSelected, required this.onTap});
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            Row(
              children: [
                CircleAvatar(radius: 16, backgroundImage: NetworkImage(contact.avatarUrl)),
                const SizedBox(width: 8),
                Text(contact.name, style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600)),
              ],
            ),
            Icon(
